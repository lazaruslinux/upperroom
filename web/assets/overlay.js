...
// recent chat, join notices, clip alerts and highlighted messages over the
// broadcast video. It cannot sign in, so it authenticates with the bearer key in
// its own URL (?key=...) and connects to the chat socket read-only. It never
// sends chat and is never counted as a viewer.
//
...
// ---- accent flavor ----
// Match the channel's brand color, the same data-accent hook the rest of the site
// uses. Read once at load: the accent is a channel setting, not live state, and a
// browser source left running for hours should not poll for it. Best effort, so a
// failed fetch simply leaves the default palette in place.
...
(async function loadAccent() {
  try {
    ...
    /* leave the default palette in place */
  }
})();
...
function connect() {
  ...
  socket.addEventListener("message", (event) => {
    ...
    // Any other type (hello, presence, theater, ...) is ignored.
  });
